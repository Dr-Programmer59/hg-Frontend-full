...
const page = ({ searchParams }) => {
    ...
    let webrtc_client = new webrtcMediaSoup_client(true, socket)
    ...
    const handleNewProducer = ({ producerId, socketId }) => {
        console.log("this is device ", webrtc_client.mystream)
        console.log("new producer enters ..producer hits.", socketId)
        console.log(producerId)
        webrtc_client.signalNewConsumerTransport(producerId)

        console.log(webrtc_client.mainTracks)
        alltracks.current = webrtc_client.mainTracks

        console.log("in new producer  this is all tracks ", alltracks.current)
        console.log(socket.id)
        console.log("this is socket id", socketId)
        if (!waitingHosts.current[socketId]) {
            waitingHosts.current[socketId] = []
        }
        if (!waitingHosts.current[socketId].includes(producerId)) { waitingHosts.current[socketId].push(producerId) }
    ...
    const handleAccept = () => {
        ...
        for (var i = 0; i < alltracks.current.length; i = i + 2) {

            var combinedStream = new MediaStream();

            // Add audio track to the combined stream
            alltracks.current[i].getAudioTracks().forEach(track => {
                combinedStream.addTrack(track);
            });

            // Add video track to the combined stream
            alltracks.current[i + 1].getVideoTracks().forEach(track => {
                combinedStream.addTrack(track);
            });

            if (!(oldTracks.current.includes(alltracks.current[i])) && !(oldTracks.current.includes(alltracks.current[i + 1]))) {
                mediaTracks.push(combinedStream)
                oldTracks.current.push(alltracks.current[i])
                oldTracks.current.push(alltracks.current[i + 1])
                console.log("this is stream check", oldTracks)
            }
        }

        console.log("all producers ", waitingHosts)
        setotherStream((prev) => [...prev, ...mediaTracks])
        console.log(" in loop this is the media stream tracks", mediaTracks)
